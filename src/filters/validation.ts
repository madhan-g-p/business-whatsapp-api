...
export function validateMessageFilter(filter: FilterTypeMessageFilter): void {
...
export function validateFilterOperator(operator: FilterOperator): void {
  ...
  if (key === 'AND' || key === 'OR') {
    if (key === 'AND') {
      ...
      list.forEach((filter: FilterTypeMessageFilter) => {
